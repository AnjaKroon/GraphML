...
class Preprocessor:
    ...
    '''
    def __init__(self, flow_dataset, epi_dataset, locations_data, k=3, skew=10, plottable=False):
        self.k = k
        self.skew = skew
        self.plottable = plottable
        self.geoid_offset = 1000000

        # load in data
        self.epidemiology = np.load(epi_dataset)
        self.locations = np.load(locations_data)
        self.flow = pd.read_csv(flow_dataset)

        self.timegraph_size = self.epidemiology.shape[0]

        # preprocess
        geoids = pd.DataFrame(self.locations.astype(np.int32), columns=['geoid_o'])
        self.epidemiology_timesteps = []
        for i, timestep in enumerate(self.epidemiology):
            timestep = pd.DataFrame(timestep, columns=['population1', 'population2', 'weather1', 'weather2',\
                                                       'cumulative_confirmed', 'cumulative_deceased', 'new_deceased',\
                                                       'cumulative_persons_fully_vaccinated', 'new_persons_fully_vaccinated','new_confirmed'])
            # vstack kronecker graph with the epidemiological data
            self.epidemiology_timesteps.append(pd.concat([timestep, geoids], axis=1))
        self.process_population_flow()

        # add geoid to epidemiology timesteps
    ...
    def combined_manual_kronecker(self):
        ...
        for i in range(self.timegraph_size - 1):
            ...
            timestep_data['geoid_o'] += self.geoid_offset * i
            timestep_data['geoid_d'] += self.geoid_offset * (i+1)
            if (self.plottable):
    ...
    def disjoint_manual_kronecker(self):
        ...
        for i in range(self.timegraph_size):
            # update epi geoid
            self.epidemiology_timesteps[i]['geoid_o'] += self.geoid_offset * i
...
if __name__ == "__main__":
    ...
    preprocessor = Preprocessor(flow_dataset, epi_dataset, locations_data, plottable=True)

    print(preprocessor.get_epi_dataset())
